...
// };
/*
  Cleaned Timesheet Controller
  - Supports multiple entries per day (Option A)
  - Enforces max 9 hours/day (rejects requests that would exceed the cap)
  - Recalculates TimeSummary after create/update/patch/delete
  - Adds delete endpoint
  - Centralized date normalization and helper functions
  - Uses existing Mongoose models: TimeEntry, TimeSummary, Leave

  Usage: require and mount as usual in your routes.
*/

/*
  Cleaned Timesheet Controller - Corrected version
  - Option A: multiple entries per day allowed
  - Enforces max 9 hours/day (rejects requests that would exceed the cap)
  - Recalculates TimeSummary after create/update/patch/delete
  - Adds delete endpoint
  - Centralized date normalization and helper functions
...
exports.updateTimeEntryByEmail = async (req, res) => {
  try {
    ...
    if (!payload.date) return res.status(400).json({ msg: 'Date is required' });
    if (payload.hours !== undefined && (typeof payload.hours !== 'number' || payload.hours <= 0)) {
      return res.status(400).json({ msg: 'hours must be a positive number' });
    }
